...
const List = () => {
  ...
  const {wisataReligi} = useSelector(state => state.categoryReducer);

  useEffect(() => {
    dispatch(getDestinationByCategory('3'));
  }, []);
...
const Maps = () => {
  ...
  useEffect(() => {
    ...
    const index = wisataReligi.findIndex(place => place.id === selectedPlaceId);

    flatList.current.scrollToIndex({index});

    const selectedPlace = wisataReligi[index];
...
export default ListTabWisataReligi;
