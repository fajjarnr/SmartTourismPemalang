...
const List = () => {
  ...
  const {hotel} = useSelector(state => state.categoryReducer);

  useEffect(() => {
    dispatch(getDestinationByCategory('5'));
  }, []);
...
const Maps = () => {
  ...
  useEffect(() => {
    ...
    const index = hotel.findIndex(place => place.id === selectedPlaceId);

    flatList.current.scrollToIndex({index});

    const selectedPlace = hotel[index];
